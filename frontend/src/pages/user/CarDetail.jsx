      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="lg:flex gap-8">
            ...
            <div className="flex gap-3 mt-4 overflow-x-auto pb-2">
              {car.images.map((img, i) => (
                <button
                  ...
                  className={`flex-shrink-0 w-20 h-14 rounded-lg overflow-hidden border-2 transition-colors ${activeImage === i ? 'border-primary-500' : 'border-transparent'}`}
                >
            ...
            {car.features?.length > 0 && (
              <div className="mt-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {car.features.map((f) => (
                    <span key={f} className="flex items-center gap-1 bg-primary-50 text-primary-700 px-3 py-1.5 rounded-full text-sm">
                      <FaCheck className="text-xs" /> {f}
                    </span>
                  ))}
                </div>
              </div>
        ...
        <div className="lg:w-2/5 mt-8 lg:mt-0">
          <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 sticky top-24">
            <div className="mb-4">
              <h1 className="text-2xl font-bold text-gray-900">{car.brand} {car.name}</h1>
              <span className="inline-block mt-1 text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">{car.category} • {car.year}</span>
            </div>
            ...
            <div className="border-t border-gray-100 pt-4 mb-4">
              <div className="text-3xl font-bold text-primary-700">{formatINR(car.pricePerDay)}<span className="text-sm font-normal text-gray-400">/day</span></div>
            </div>
            ...
            {totalDays > 0 && (
              <div className="bg-gray-50 rounded-lg p-4 mb-4">
                ...
                <div className="flex justify-between font-bold text-lg text-gray-900 pt-2 border-t border-gray-200 mt-2">
                  <span>Total</span>
                  <span className="text-primary-700">{formatINR(totalPrice)}</span>
                </div>
              </div>
            )}
